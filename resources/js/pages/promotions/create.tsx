...
export default function Create({ users, artists }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        artist_id: artists.length > 0 ? artists[0].id : 0,
        track_title: '',
        intermediary_name: '',
        intermediary_contact: '',
        intermediary_channel: 'FB',
        package_tier: 'Premium',
        boost_count: 0,
        amount_paid: '',
        payment_method: 'OrangeMoney',
        receiver_number: '0322616963',
        transaction_id: '',
        manager_id: users.length > 0 ? users[0].id : 0,
        status: 'PENDING' as 'PENDING' | 'ACTIVE' | 'DONE',
    });
